...
final class OwnerDashboardService
{
    ...
    {
        $academyIds = DB::table('academies')->where('owner_id', $owner->id)->pluck('id');

        if ($academyIds->isEmpty()) {
            return [
                'owner'=>$owner,'academies'=>collect(),
                'metrics'=>['courses'=>0,'teachers'=>0,'students'=>0,'classrooms'=>0,'sales'=>0,'published'=>0,'pendingReviews'=>0],
                'teacherReports'=>collect(),'recentCourses'=>collect(),'upcomingLiveClasses'=>collect(),
            ];
        }

        $courseIds = Course::query()->whereIn('academy_id',$academyIds)->pluck('id');
        $teacherIds = DB::table('academy_user')->whereIn('academy_id',$academyIds)->where('role','teacher')->where('status','active')->distinct()->pluck('user_id');
        $studentIds = DB::table('academy_user')->whereIn('academy_id',$academyIds)->where('role','student')->where('status','active')->distinct()->pluck('user_id');

        $revenue = (float) DB::table('financial_transactions')->whereIn('academy_id',$academyIds)->where('status','completed')->where('type','enrollment_payment')->sum('amount');
        $refunds = (float) DB::table('financial_transactions')->whereIn('academy_id',$academyIds)->where('status','completed')->where('type','refund')->sum('amount');

        $metrics = [
            'courses'=>$courseIds->count(),
            'teachers'=>$teacherIds->count(),
            'students'=>$studentIds->count(),
            'classrooms'=>DB::table('classrooms')->whereIn('academy_id',$academyIds)->where('status','active')->count(),
        ...
        return [
            'owner'=>$owner,'academies'=>$owner->ownedAcademy()->get(),
            'metrics'=>$metrics,
            'teacherReports'=>$teacherReports,
            'recentCourses'=>Course::query()->whereIn('id',$courseIds)->with('academy:id,name')->withCount([
            ...
            'upcomingLiveClasses'=>DB::table('live_classes')
                ...
                ->limit(8)
                ->get(['live_classes.id','live_classes.title','live_classes.scheduled_at','courses.title as course_title','classrooms.title as classroom_title','teachers.name as teacher_name']),
        ];
    }
}