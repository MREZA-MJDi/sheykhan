...
<div class="mx-auto max-w-3xl">
    <div class="dashboard-panel p-5 sm:p-7">
        ...
        <form method="POST" action="{{ $liveClass->exists ? route('teacher.live-classes.update',$liveClass) : route('teacher.live-classes.store') }}" class="mt-6 grid gap-5 sm:grid-cols-2">
            ...
            <label class="grid gap-2 sm:col-span-2"><span class="text-xs font-bold">دوره</span><select name="course_id" required data-course-scope-select data-classroom-target="live-classroom" class="rounded-xl border border-slate-200 px-3 py-3">@foreach($courses as $course)<option value="{{ $course->id }}" @selected((string) old('course_id',$liveClass->course_id)===(string)$course->id)>{{ $course->title }}</option>@endforeach</select></label>
            <label class="grid gap-2"><span class="text-xs font-bold">کلاس</span><select name="classroom_id" id="live-classroom" class="rounded-xl border border-slate-200 px-3 py-3"><option value="">عمومی دوره</option>@foreach($classrooms as $classroom)<option value="{{ $classroom->id }}" data-course-id="{{ $classroom->course_id }}" @selected((string) old('classroom_id',$liveClass->classroom_id)===(string)$classroom->id)>{{ $classroom->title }} · {{ $classroom->course?->title }}</option>@endforeach</select></label>
