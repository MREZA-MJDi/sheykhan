...
<div class="mx-auto max-w-3xl">
    <div class="dashboard-panel p-5 sm:p-7">
        ...
        <form method="POST" action="{{ $assignment->exists ? route('teacher.assignments.update', $assignment) : route('teacher.assignments.store') }}" class="mt-6 grid gap-5">
            ...
            <label class="grid gap-2"><span class="text-xs font-bold">دوره</span><select name="course_id" required class="rounded-xl border border-slate-200 px-3 py-3 text-sm">@foreach($courses as $course)<option value="{{ $course->id }}" @selected((string) old('course_id',$assignment->course_id) === (string) $course->id)>{{ $course->title }}</option>@endforeach</select></label>
            <label class="grid gap-2"><span class="text-xs font-bold">کلاس</span><select name="classroom_id" class="rounded-xl border border-slate-200 px-3 py-3 text-sm"><option value="">همه دانش‌آموزان دوره</option>@foreach($classrooms as $classroom)<option value="{{ $classroom->id }}" @selected((string) old('classroom_id',$assignment->classroom_id) === (string) $classroom->id)>{{ $classroom->title }} · {{ $classroom->course?->title }}</option>@endforeach</select></label>
